...
class Woocommerce_Addons {
	...
	public static function add_to_cart() {
		...
		if ( $product && ( 'variation' === $product->get_type() || 'subscription_variation' === $product->get_type() ) ) {
			...
			//for woocommerce subscription upgrades (switches), there's some neccessary data that subscriptions is expecting to be in the url params (_GET)
			//we don't pass those that way for ajax, so lets put them in there if they exist. 
			$switch_subscription = empty( $_POST['switch-subscription'] ) ? '' : wp_unslash( $_POST['switch-subscription'] );
			$item = empty( $_POST['item'] ) ? '' : wp_unslash( $_POST['item'] );
			if ( $switch_subscription && $item ) {
				$_GET['switch-subscription'] = $switch_subscription;
				$_GET['item'] = $item;
			}
		}
